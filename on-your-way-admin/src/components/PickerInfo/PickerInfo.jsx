...
function PickerInfo({ image, email, name, phone, address, rate, close }) {
  return (
    <div className="fixed top-0 left-0  bg-[rgba(0,0,0,0.8)] h-screen w-screen z-[1000]">
      <div className=" absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  p-5 bg-primary  h-auto w-1/2   rounded-lg">
        <button
          onClick={close}
        ...
        <div className="flex flex-col  items-center  justify-center h-full w-full">
          <div className="w-1/2 h-1/4 flex flex-col items-center justify-center mx-2 ">
            <img src={image} alt="userImage" className="mb-3 w-40 h-40 rounded-full" />
            <Rate rate={rate} size={24} />
          </div>
          <div className="w-3/4 h-full flex flex-col  items-center justify-center mx-2">
            <InputBox
              type="text"
              value={name}
